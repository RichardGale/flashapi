#if !defined(FLEX11_6_FLASH_EVENTS_IEVENTDISPATCHER_AS)
#define FLEX11_6_FLASH_EVENTS_IEVENTDISPATCHER_AS
...
namespace flash
{
    namespace events
    {
        class IEventDispatcher
        {
             ...
             * priority level.After the listener is registered, subsequent calls to
             ...
             * listeners attached to it move along with it.If the event listener is being registered on a node while an event is also being processed on
             * this node, the event listener is not triggered during the current phase but may be
             * triggered during a later phase in the event flow, such as the bubbling phase.If an event listener is removed from a node while an event is being processed on the node, it is still triggered by the current actions. After it is removed, the event listener is never invoked again
...
#endif // FLEX11_6_FLASH_EVENTS_IEVENTDISPATCHER_AS
#endif // __cplusplus
