#if !defined(FLEX11_6_FLASH_SYSTEM_LOADERCONTEXT_AS)
#define FLEX11_6_FLASH_SYSTEM_LOADERCONTEXT_AS
 ...
 * <codeph class="+ topic/ph pr-d/codeph ">applicationDomain</codeph> and <codeph class="+ topic/ph pr-d/codeph ">securityDomain</codeph> properties.</p><p class="- topic/p ">When loading a SWF file with the <codeph class="+ topic/ph pr-d/codeph ">Loader.loadBytes()</codeph> method, you have the same
 ...
 * places its loaded SWF file into the security domain of the loading SWF file.</p><p class="- topic/p ">When loading images (JPEG, GIF, or PNG) instead of SWF files, there is no need to
...
namespace flash
{
    namespace system
    {
        class LoaderContext: public Object
        {
             ...
             * true, the application does not begin downloading the specified object in URLRequest.url
             ...
             * on the same server as URLRequest.url. In all cases, the given policy file is required to exist
             * at URLRequest.url by virtue of the policy file's location, and the file must permit access
             * by virtue of one or more <allow-access-from>
             * tags.If you set checkPolicyFile to true, the main download that specified in the
             * Loader.load() method does not load until the policy file has been completely processed.
             ...
             * URL that you specify in URLRequest.url. If the final
             ...
             * URL. Then poll the value of LoaderInfo.childAllowsParent until it becomes true.You do not need to set this property for AIR content running in the application sandbox. Content
             * in the AIR application sandbox can call the BitmapData.draw() method using any loaded image
        ...
        public:
             ...
             * as a class already defined by the parent, the parent class is used and the child class is ignored.Child of the system ApplicationDomain.  You use this application domain when using
             ...
             * Environment" chapter of the ActionScript 3.0 Developer's Guide.
             * @langversion 3.0
             * @playerversion   Flash 9
             * @playerversion   Lite 4
             */
        public:
             ...
             * Attempting to pass any other security domain results in a SecurityError exception.Content in the AIR application security sandbox cannot load content from
             * other sandboxes into its SecurityDomain.For more information, see the "Security" chapter in the ActionScript 3.0 Developer's Guide.
             * @langversion 3.0
             * @playerversion   Flash 9
        ...
        public:
             ...
             * the Loader.loadBytes() method, and the Loader.load() method with
             * LoaderContext.securityDomain = SecurityDomain.currentDomain.  (The latter operation is not
             * supported in the AIR application sandbox.)  With the allowCodeImport property set to
             * false, these importing operations are restricted to safe operations, such as loading
             * images.  Normal, non-importing SWF file loading with the Loader.load() method is not
             * affected by the value of this property.
             *
             *   This property is useful when you want to import image content into your sandbox - for example, when you
             ...
             * importing a SWF file is a much riskier operation than importing an image file.In AIR content in the application sandbox, the default value is false.  In non-application
             * content (which includes all content in Flash Player), the default value is true.The allowCodeImport property was added in Flash Player 10.1 and AIR 2.0.
             * However, this property is made available to SWF files and AIR applications of all versions when the
             * Flash Runtime supports it.
             * @langversion 3.0
        ...
        public:
             ...
             * If requestedContentParent is set, the object that it specifies becomes the parent, unless a runtime error prevents the assignment.
             * This reparenting can also be done after the complete event without use of this property. However, specifying the
             * parent with LoaderContext.requestedContentParent eliminates extra events. LoaderContext.requestedContentParent sets the desired parent before frame one scripts in
             * the loaded content execute, but after the constructor has run. If requestedContentParent is null (the default),
             * the Loader object becomes the content's parent.If the loaded content is an AVM1Movie object, or if an
             * error is thrown when addChild() is called on the requestedContentParent object, then the following actions occur:
             * The Loader object becomes the parent of the loaded content.The runtime dispatches an AsyncErrorEvent.If the requested parent and the loaded content are in different security sandboxes, and if the requested parent does not have access
             * to the loaded content, then the following actions occur:
             * The Loader becomes the parent of the loaded content.The runtime dispatches a SecurityErrorEvent.The following code uses requestedContentParent to place the loaded content into a Sprite object:
             ...
             * When this code runs, the child SWF file appears on stage. This fact confirms that the Sprite object you added to the stage is the
             * parent of the loaded child.swf file.
             * @langversion 3.0
             * @playerversion   Flash 10.2
             * @playerversion   AIR 2.6
             */
        public:
            flash::display::DisplayObjectContainer *requestedContentParent;

            /**
             * An Object containing the parameters to pass to the LoaderInfo object of the content.
             *
             *   Normally, the value of the contentLoaderInfo.parameters property is obtained by parsing the requesting URL.
             * If the parameters var is set, the contentLoaderInfo.parameters gets its value from the LoaderContext object, instead of from
             * the requesting URL. The parameters var accepts only objects containing name-value string pairs, similar to URL parameters. If the object does
             * not contain name-value string pairs, an IllegalOperationError is thrown.The intent of this API is to enable the loading SWF file to forward its parameters to a loaded SWF file.
             * This functionality is especially helpful when you use the loadBytes() method, since LoadBytes does not provide a
             * means of passing parameters through the URL. Parameters can be forwarded successfully only to another SWF file; an AS1 or AS2 SWF file cannot receive the parameters in an
             * accessible form, although the AVM1Movie's loaderInfo.parameters object will be the forwarded object. For example, consider the following URL: http://yourdomain/users/jdoe/test01/child.swf?foo=bar;The following code uses the LoaderContext.parameters property to replicate a parameter passed to this URL:
             * import flash.system.LoaderContext;
             * import flash.display.Loader;
             * var l:Loader = new Loader();
             * var lc:LoaderContext = new LoaderContext;
             ...
             * l.load(new URLRequest("child.swf"), lc);
             * To verify that the parameter passed properly, use the following trace statement after you run this code:trace(loaderInfo.parameters.foo);If the content loaded successfully, this trace prints "bar".
             * @langversion 3.0
             * @playerversion   Flash 10.2
             * @playerversion   AIR 2.6
             */
        public:
             ...
             *
             *   Under the default policy, ImageDecodingPolicy.ON_DEMAND, the runtime decodes the image data
             * when the data is needed for display or other purpose. This policy maintains the decoding behavior used
             * by previous versions of the runtime.Under the ImageDecodingPolicy.ON_LOAD policy, the runtime decodes the image immediately after
             ...
             * be the case under the onDemand policy.Under both policies, the runtime uses the same cache and flush behavior after the image is decoded.
             ...
             */
        public:
            std::string imageDecodingPolicy;

            /**
             * Legacy property, replaced by allowCodeImport, but still supported for compatibility.
             * Previously, the only operation affected by allowLoadBytesCodeExecution was the
             * Loader.loadBytes() method, but as of Flash Player 10.1 and AIR 2.0, the import-loading
             * operation of Loader.load() with LoaderContext.securityDomain = SecurityDomain.currentDomain
             * is affected as well.  (The latter operation is not supported in the AIR application sandbox.)
             * This dual effect made the property name allowLoadBytesCodeExecution overly specific,
             * so now allowCodeImport is the preferred property name.  Setting either of
             * allowCodeImport or allowLoadBytesCodeExecution will affect the value of both.
             *
             *   Specifies whether you can use a Loader object to import content with executable code,
             * such as a SWF file, into the caller's security sandbox.  With this property set to false,
             * these importing operations are restricted to safe operations, such as loading images.In AIR content in the application sandbox, the default value is false. In non-application content, the
        ...
        public:
             ...
             * @param   checkPolicyFile Specifies whether a check should be made for the existence
...
#endif // FLEX11_6_FLASH_SYSTEM_LOADERCONTEXT_AS
#endif // __cplusplus
